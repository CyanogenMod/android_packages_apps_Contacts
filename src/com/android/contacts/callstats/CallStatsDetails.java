...
public class CallStatsDetails {
    ...
    public CallStatsDetails(CharSequence number, CharSequence formattedNumber,
        ...
        this(number, formattedNumber, countryIso, geocode, date, name,
                numberType, numberLabel, contactUri, photoUri, photoId, 0, 0, 0, 0, 0);
    }
    ...
    public int getInPercentage() {
        return Math.round((float) inDuration * 100 / (float) getFullDuration());
    }

    public int getOutPercentage() {
        return Math.round((float) outDuration * 100 / (float) getFullDuration());
    }
