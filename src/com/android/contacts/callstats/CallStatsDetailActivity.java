...
public class CallStatsDetailActivity extends Activity {
    ...
    protected void onCreate(Bundle icicle) {
        ...
        mCallStatsDetailHelper = new CallStatsDetailHelper(mResources, mPhoneNumberHelper);
        ...
        if (filterFrom == -1) {
            ...
            mDateFilterView.setText(DateUtils.formatDateRange(this,
                    filterFrom, filterTo, DateUtils.FORMAT_ABBREV_ALL));
        }
    }
    ...
    private void updateData() {
        ...
        if (canPlaceCallsTo) {
            final CharSequence displayNumber = mPhoneNumberHelper.getDisplayNumber(
                    mData.number, mData.formattedNumber);
            ...
            if (!TextUtils.isEmpty(mData.name)
                ...
                entry.label = Phone.getTypeLabel(mResources, mData.numberType, mData.numberLabel);
            }
        ...
        mTotalText.setText(getString(R.string.call_stats_header_total_details,
                CallStatsDetailHelper.getDurationString(this, mData.getFullDuration(), true)));

        if (mData.inDuration != 0) {
            mInText.setText(getString(R.string.call_stats_incoming, mData.getInPercentage()));
            mInText2.setText(CallStatsDetailHelper.getDurationString(this, mData.inDuration, true));
            mPieChart.addSlice(mData.inDuration, mResources.getColor(R.color.call_stats_incoming));
        } else {
            findViewById(R.id.in_container).setVisibility(View.GONE);
        }

        if (mData.outDuration != 0) {
            mOutText.setText(getString(R.string.call_stats_outgoing, mData.getOutPercentage()));
            mOutText2.setText(CallStatsDetailHelper.getDurationString(this, mData.outDuration, true));
            mPieChart.addSlice(mData.outDuration, mResources.getColor(R.color.call_stats_outgoing));
        } else {
        ...
        if (mData.missedCount != 0) {
            mMissedText.setText(getString(R.string.call_stats_missed,
                    CallStatsDetailHelper.getCallCountString(this, mData.missedCount)));
        } else {
